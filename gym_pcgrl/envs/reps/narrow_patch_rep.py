...
from gym_pcgrl.envs.probs.loderunner.helper_lr import get_actions
...
"""
class NarrowPatchRepresentation(Representation):
    """
    ...
    def reset(self, width, height, prob):
        ...
        self._x = self._random.randint(width-1)
        self._y = self._random.randint(height-1)        
        self._player_x, self._player_y = self.set_player() 
    ...
    def update(self, action):
        change = 0
        if action > 0:
            #get the action-patch from the index
            action_str = self._actions[action-1]
            #converst the str to array
            tmp = action_str.split('\n')
            action_patch =[]
            for t in tmp:
                t = t.replace('[','').replace(']','')
                action_patch.append(t.split())
            action_patch =  np.asarray(action_patch).astype(np.uint8)                        
            curr = self._map[self._y:self._y+2,self._x:self._x+2]
            #do not change the player tile
            if 7 in curr:
                r, c = np.where(curr == 7)
                action_patch[r,c] = 7
                
            change += [1,0][np.array_equal(action_patch,curr)] 
            self._map[self._y:self._y+2,self._x:self._x+2] = action_patch
        if self._random_tile:
            self._x = self._random.randint(self._map.shape[1]-1)
            self._y = self._random.randint(self._map.shape[0]-1)
        else:
            self._x += 1
    ...
    Returns:
        int,int: Player location
    """     
    def set_player(self):
        import random
        #replace player tiles(if any) by empty
        self._map[self._map == 7] = 0
        y = self._random.randint(self._map.shape[0])
        x = self._random.randint(self._map.shape[1])
        self._map[y,x] = 7
        return x ,y
